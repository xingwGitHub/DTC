...
var Stats = function(
  ...
  container.style.cssText = `position:fixed;top:${option.y ||
    0}px;left:${option.x || 0}px;cursor:pointer;opacity:0.9;z-index:10000`
  ...
  function move(event) {
    ...
    document.onmousemove = function(event) {
      ...
      if (
        parseInt(tx + (option.x || 0)) >=
        window.innerWidth - parseInt(getComputedStyle(container).width)
      ) {
        tx =
          window.innerWidth -
      ...
      if (
        parseInt(ty + (option.y || 0)) >=
        window.innerHeight - parseInt(getComputedStyle(container).height)
      ) {
        ty =
          window.innerHeight -
