...
export default class Scrollload {
  ...
  constructor(options = {}) {
    ...
    const container =
      this._options.container || document.querySelector('.scrollload-container')
    ...
    this.contentDom =
      ...
      this.container.querySelector('.scrollload-content')
  ...
  createTopDom() {
    ...
    this.notEnoughRefreshPortDom = topContentDom.querySelector(
      '.scrollload-notEnoughRefreshPort'
    )
    this.overRefreshPortDom = topContentDom.querySelector(
      '.scrollload-overRefreshPort'
    )
  ...
  isTop() {
    return this.isGlobalScroll
      ? window.pageYOffset <= 0
      : this.win.scrollTop <= 1
  }
  ...
  isArrivedRefreshPort() {
    const preDistance = this._options.calMovingDistance(
      this.prePageY - this.startPageY
    )
    return (
      (this.distance >= this.topContentDomHeight &&
        preDistance < this.topContentDomHeight) ||
      (this.distance <= this.topContentDomHeight &&
        preDistance > this.topContentDomHeight)
    )
  }
  ...
  touchStart(event) {
    ...
    // 在滑动的时候是不需要过渡动画的
    setStyles(
      [this.topDom, this.contentDom, this.bottomDom, this.topContentDom],
      {
        transition: 'none'
      }
    )
