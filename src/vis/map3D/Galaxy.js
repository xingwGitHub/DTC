...
class Galaxy {
  constructor(o) {
    ...
    let sun = createStar({
      ...
      size: 20,
      emissive: 0xe65f05,
      map: THREE.ImageUtils.loadTexture('static/sun.jpg'),
      revspeed: 0.01, //公转速度
      rotspeed: 0.01 //自转速度
    })
    this.stars.push(sun)

    let suixin = createStar({
      name: '水星',
      size: 3,
      dist: 40,
      emissive: 0xe65f05,
      map: THREE.ImageUtils.loadTexture('static/shuixin.png'),
      revspeed: 0.01,
      rotspeed: 0.01
    })
    this.stars.push(suixin)

    let jinxin = createStar({
      name: '金星',
      color: 0xcceeff,
      dist: -60,
      mateType: 2,
      map: THREE.ImageUtils.loadTexture('static/jinxin.png'),
      revspeed: 0.007,
      rotspeed: 0.01
    })
    ...
    let s2 = createStar({
      ...
      dist: 80,
      mateType: 2,
      map: THREE.ImageUtils.loadTexture('static/world.jpg'),
      revspeed: 0.007,
      rotspeed: 0.01
    })
    s2.rotation.x = 0.3
    ...
    let s3 = createStar({
      ...
      dist: -100,
      mateType: 2,
      map: THREE.ImageUtils.loadTexture('static/a.jpg'),
      revspeed: 0.005,
      rotspeed: 0.01
    })
    this.stars.push(s3)
    ...
    let s4 = createStar({
      ...
      dist: 120,
      mateType: 2,
      map: THREE.ImageUtils.loadTexture('static/muxin.png'),
      revspeed: 0.003,
      rotspeed: 0.01
    })
    this.stars.push(s4)
    ...
    let s5 = createStar({
      ...
      dist: -140,
      mateType: 2,
      map: THREE.ImageUtils.loadTexture('static/tuxin.png'),
      revspeed: 0.01,
      rotspeed: 0.02
    })
    this.stars.push(s5)
    ...
    let s6 = createStar({
      ...
      dist: 160,
      mateType: 1,
      map: THREE.ImageUtils.loadTexture('static/tianwangxin.png'),
      revspeed: 0.01,
      rotspeed: 0.02
    })
    this.stars.push(s6)
    ...
    let s7 = createStar({
      ...
      dist: -180,
      mateType: 1,
      map: THREE.ImageUtils.loadTexture('static/haiwangxin.png'),
      revspeed: 0.006,
      rotspeed: 0.02
    })
    this.stars.push(s7)
  ...
  renderScene() {
    ...
    if (!isSelectedStar) {
      this.stars.forEach(star => {
        ...
        star.position.set(
          starOpt.dist * Math.sin(starOpt.positionOffset),
          0,
          starOpt.dist * Math.cos(starOpt.positionOffset)
        ) //公转
      })
    }
...
function createStar(o) {
  ...
  if (opt.line) {
    ...
    let track = new THREE.Mesh(
      new THREE.RingGeometry(opt.dist - 0.2, opt.dist + 0.2, 64, 1),
